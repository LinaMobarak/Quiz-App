...
const Result = () => {
  ...
  return (
    ...
    <div className='flex flex-col items-center justify-center'>
        ...
        <h2 className='text-2xl font-semibold'>Your Score: {useQuizStore.getState().score}</h2>
        {/* <p className='text-xl'>{useQuizStore.getState().score}</p> */}
        
        {allQuestions.map((question, index) => (
            <div key={index} className='border p-4 m-2 w-1/2'>
                <h3 className='text-lg font-semibold'>{question.question}</h3>
                <p style={{backgroundColor: 'green'}}className='text-sm'>Correct Answer: {question.answer}</p>
